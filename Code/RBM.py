...
class RBM(nn.Module):
    ...
    def contrastive_divergence(self, input_data, training=True,
        ...
        positive_associations = torch.matmul(input_data.t(), positive_hid_dis) # -- E
        ...
        for i in range(n_gibbs_sampling_steps):
            vis_prob, _ = self.to_visible(hidden_activations) # -- h→v
            hid_prob, hidden_activations = self.to_hidden(vis_prob) # -- v→h
        ...
        # Update parameters   batch_size、g、grad_update、v_bias_update、h_bias_update
        grad_update = 0
        if training:
            batch_size = self.batch_size
            g = positive_associations - negative_associations # --(E.positive - E.negative)
            ...
            self.weight.data += lr * grad_update  # -- w
            self.v_bias.data += lr * v_bias_update  # -- a
            self.h_bias.data += lr * h_bias_update  # -- b
